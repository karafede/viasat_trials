...
H.graph = G2.graph
'''
#G = ox.project_graph(G2, to_crs={'init': 'epsg:4326'})
       
#G2 = nx.Graph(G1)
from_n=np.random.choice(G2.nodes)
to_n=np.random.choice(G2.nodes)  
orig_xy = (41.889767, 12.566119)
from_n = ox.get_nearest_node(G2, orig_xy, method='euclidean')
target_xy = (41.853191, 12.604730)
to_n = ox.get_nearest_node(G2, target_xy, method='euclidean')
route = nx.shortest_path(G2,from_n,to_n,weight='cost')
ox.plot_graph_route(G2, route, fig_height=12, fig_width=7)
#fig, ax = ox.plot_graph(G2)
'''
area = ox.gdf_from_place('Rome, Italy')
buildings = ox.buildings_from_place('Rome, Italy')
...
ox, oy=pj.wgs84toutm32(orig_xy[1], orig_xy[0])
dx, dy=pj.wgs84toutm32(target_xy[1], target_xy[0])
...
ory=[int(orig[0])]
print(type(orig), orx)
fm=ox.get_nearest_nodes(G2, orx, ory, method='kdtree')
print(fm)
from_n = ox.get_nearest_node(G2, orig, method='euclidean')
fm=ox.get_nearest_nodes(G2, orig[1], orig[0])

to_n = ox.get_nearest_node(G2, dest, method='euclidean')
